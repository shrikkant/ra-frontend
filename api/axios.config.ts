...
export class HttpService {
  ...
  constructor(url: string) {
    ...
    this.client.interceptors.response.use(async function (res: AxiosResponse<any>) {
...
httpClient.interceptors.response.use(
  async function <T>(
    res: AxiosResponse<{ resultFormatted: T; successMessage?: string; errorMessage?: string }>
  ): Promise<T> {
    const response = res;
    if (response.data?.successMessage) {
      displayMessage('success', response.data.successMessage);
