...
export const addToCart = async (productId: number, dates: Dates): Promise<IOrder> => {
...
export const createRazorPayOrder = async (razorPayOrderDetails) => {
  const response = await  httpClient.post('/user/payment/razorPayOrder', razorPayOrderDetails);
...
export const updateDeliveryAddress = async (orderId, address)=> {
  ...
  const response: IOrder =  await httpClient.put("/user/orders/" + orderId + "?mode=1", address)
  console.log("Response : ", response);
