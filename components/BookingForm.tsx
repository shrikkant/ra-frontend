...
export default function BookingForm({
  ...
  const discountedRate = Math.ceil(
    rates[0].rate - (rates[0].rate * finalDiscount) / 100,
  )
      ...
      <BookingLineItem amount={discountedRate * getDays()}>
        ...
        <span>
          <span className="text-md text-gray-500 ">
            X {getDays() + ' ' + getPlural('day', getDays())}
          </span>
        </span>
      </BookingLineItem>

      {finalDiscount > 0 && (
        <BookingLineItem amount={getSavings()}>
          <span className="text-md font-semibold flex">You Save</span>
        </BookingLineItem>
      )}

      <BookingLineItem amount={discountedRate * getDays()} primary={true}>
        <span className="text-md font-semibold flex">Total</span>
      </BookingLineItem>
    </>
  )
      ...
      {isClient && (
        <div className="">
          <div className="md:block hidden bg-white shadow-md  p-5 text-sm h-full w-full  border border-1 border-gray-200 ">
            <div className="flex md:flex-col gap-4">{renderForm}</div>
          </div>

          {!openFormInMobile ? (
            <div
              className={`fixed p-4 bottom-0 left-0 right-0 bg-white md:hidden py-3 text-center border-t border-gray-300 cursor-pointer flex justify-between z-40 ${showSignIn ? 'hidden' : ''}`}
            >
              <div className="relative flex items-center">
                <span className="absolute text-md top-2"></span>
