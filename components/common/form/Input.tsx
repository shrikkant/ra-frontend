  ...
  const label = () => {
    if (props.label) {
      return (
        <label
          className={
            'text-gray-700  bg-gradient-to-t from-white  text-sm pl-1 text-left z-10 pr-1  block ' +
            (props.value && props.value.length > 0 ? 'visible' : 'invisible')
          }
          htmlFor={props.name}
        >
          {props.label}
  ...
  return (
    <div>
      ...
      <div className="relative">
        ...
        {props.error && (
          <div className="text-sm text-[#E03546] ml-2 w-full text-left">
            {props.error}
          </div>
        )}
      </div>
