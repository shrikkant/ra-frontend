...
const defaultAddons: IAddOnItem[] = [{id: 1, label: 'Camera'}]
  ...
  return (
    <div className="mt-4">
      <div className="flex  text-gray-700 text-sm font-bold mb-2 items-center justify-center">
        <div className="flex-1">Addons</div>
        <div>
        ...
        addons.map(addon => (
          ...
          >
            <div className="flex-1">
              <Input
                label={''}
                placeholder={addon.label + ' : ' + addon.id}
                value={addon.label}
                onChange={e => onAddonChange(addon.id, e)}
              />
            </div>
            <div
              className={'mb-4 cursor-pointer'}
              onClick={() => removeItem(addon)}
            >
