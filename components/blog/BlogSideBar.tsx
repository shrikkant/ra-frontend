...
export default function BlogSideBar({ blogs, type = ARTICLE_TYPES.BLOG }: BlogSideBarProps) {
  ...
  return (<>
    ...
    <ul className="widgets">
      ...
      <li className="widget widget-recent-posts">
        <h5 className="title">
          {ARTICLE_TYPES.HELP_ARTICLE === type ? "Help Articles" : "Recent Posts"}
        </h5>
        <ul>
          {blogs && blogs.map((blog) =>
            <li key={blog.id}>
              <a href={`/${type === ARTICLE_TYPES.HELP_ARTICLE ? "help" : "blog"}/${blog.slug}`}>{blog.title}</a>
