...
export function ItemDiscountForm({
  item,
  handleItemChange,
}: {
  item: IOrderItem
  handleItemChange: (item: IOrderItem) => void
}) {
  ...
  const handleSubmit = async e => {
    e.preventDefault()
    const order: IOrder = await applyDiscount(
      item.order_id,
      item.id,
      transactionUpdate,
    )
  ...
  return (
    <form className="w-40">
      ...
      <div className="flex justify-end w-full py-2">
        <button type="button" onClick={handleSubmit} className="h-8 p-2">
          Apply
        </button>
      </div>
    </form>
