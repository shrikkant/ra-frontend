...
export function OrderStageForm({order}: {order: IOrder}) {
  ...
  const handleSerialNoInput = async (transactionId, addon, value) => {
    ...
    const alreadyExists: any = await orderUpdate.serialNoInfo.find(
      item => item.id == transactionId,
    )

    if (alreadyExists) {
      let serialInfo = alreadyExists?.serial_no_json.find(
      ...
      <form>
        <div className="flex items-center justify-between gap-x-2">
          <div>
            ...
            <select
              value={String(orderChange.stage)}
          ...
          <div>
            <button
              type="button"
              className="bg-gray-300 hover:bg-gray-400 text-gray-800 font-bold py-1 px-4 rounded inline-flex items-center h-8"
        ...
        {order.stage !== OrderStages.InProgress &&
          ...
          order.items?.map(transaction => {
            return (
              transaction.product.masterProductList && (
                      ...
                      addon && (
                        <div key={addon?.id}>
                          <Input
                            ...
                            onChange={e => {
                              handleSerialNoInput(transaction.id, addon, e)
                            }}
                          />
                        </div>
                      )
